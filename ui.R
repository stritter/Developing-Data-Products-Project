...
shinyUI(
  pageWithSidebar(
    ...
    mainPanel(
        ...
        h3('Documentation'),
        p('Compare the empirical with theoretical coverage probalility.'),
        p('The app calculates the empirical coverage of a number of samples (number of simulations). The empirical mean and (1-alpha) % confidence intervall of each sample is calculated. Then the app counts how often the theoretical mean (1/lambda), population mean of the exponential distribution, is in the calculated confidence intervall of each sample. The percentage,i.e. the count that the population mean is covered by the confidence intervall divided by the number of simulations, is the result the app displays in the main panel.'),
        ...
        p('The main panel shows the values for the variables and the empirical coverage of the simulation after the submit button is pressed.'),
